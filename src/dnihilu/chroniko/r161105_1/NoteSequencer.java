...
//	long loadedTick[];
	...
	public void prime() {
		...
		for(int i = 0; i < board.length; i++){
			...
			while(!finished[i] && getLoadedTime(i) < board[0].getLength() + 1)
				parseNext(i);
	...
	public void refresh(){
		for(int i = 0; i < board.length; i++){
			if(!finished[i] && (getLoadedTime(i) < board[i].getLength() + board[i].window.music.getTime() + 1)){
				parseNext(i);
			}
		}
	}
	...
	*/
	...
	//			System.out.println("Strt "+lane+" "+getTime(tick));
			...
			int lane = m[1]-offset[i];
			if(lane < startedNote[i].length && lane >= 0){
				board[i].addNote(getTime(startedNote[i][lane]), getTime(tick), lane, getNoteLength(tick-startedNote[i][lane]));
...
/*	private void setLoadedTick(int i, double ter){
		this.loadedTime[i] = ter;
	}
	
	private double getLoadedTick(int i){
		return loadedTick[i];
	}*/
